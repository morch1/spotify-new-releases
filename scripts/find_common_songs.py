...
def run(token, username, playlist1, playlist2, by_name, find_missing):
    ...
    while songs:
        for song in songs['items']:
            on_both = song['track']['id'] in tracks1 or (by_name and any(s1[0] == song['track']['artists'][0]['id'] and s1[1] == song['track']['name'] for _, s1 in tracks1.items()))
    ...
    if len(common_tracks) > 0:
        result_playlist = sp.user_playlist_create(username, f'find_common_songs result {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}')
