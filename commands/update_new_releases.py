...
def run(config, playlist_id, num_days=30):
    ...
    tbl_artists = config.spotify.TABLE_FOLLOWED_ARTISTS
    tbl_releases = config.spotify.TABLE_NEW_RELEASES
    ...
    c.execute(f'SELECT id FROM {tbl_artists}')
    db_followed_artists = [r[0] for r in c.fetchall()]
    for artist in db_followed_artists:
        if artist not in followed_artists:
            c.execute(f'DELETE FROM {tbl_artists} WHERE id = ?', (artist,))
            c.execute(f'DELETE FROM {tbl_releases} WHERE artist = ?', (artist,))
    ...
    for i, (artist, artist_name) in enumerate(followed_artists.items()):
        time.sleep(0.2)
        c.execute(f'SELECT id FROM {tbl_artists} WHERE id = ?', (artist,))
        new_artist = c.fetchone() is None
        if new_artist:
            c.execute(f'INSERT INTO {tbl_artists} (id, name) values (?, ?)', (artist, artist_name))
            ...
            for album in albums['items']:
                ...
                while songs:
                    for track in songs['items']:
                        ...
                        c.execute(f'SELECT release_date FROM {tbl_releases} WHERE artist = ? AND track_name = ?', (artist, track_name))
                        db_release_date = (c.fetchone() or (None,))[0]
                        if db_release_date is None or release_date < db_release_date:
                            c.execute(f'DELETE FROM {tbl_releases} WHERE artist = ? AND track_name = ?', (artist, track_name))
                            c.execute(f'INSERT INTO {tbl_releases} (artist, track, album, release_date, track_name, playlisted) VALUES (?, ?, ?, ?, ?, ?)',
            ...
            for album in albums['items']:
                ...
                while songs:
                    for track in songs['items']:
                        ...
                        c.execute(f'SELECT id FROM {tbl_releases} WHERE artist = ? AND track_name LIKE ?', (artist, short_name + '%'))
    ...
    c.execute(f'SELECT track, album, release_date FROM {tbl_releases} WHERE release_date > ? AND playlisted = ?', (cutoff_date, 0))
    r = c.fetchall()
    ...
    for i, (album, (release_date, album_track_ids)) in enumerate(new_albums.items()):
        ...
        c.execute(f'UPDATE {tbl_releases} SET playlisted = ? WHERE album = ?', (1, album))
        bar.update(i)
