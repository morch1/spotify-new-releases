...
def run(config, playlist_id, src_playlist_id, liked_only=False, retain_order=False):
    ...
    if retain_order:
        ...
        to_add = [t for t in src_tracks if t not in playlisted_tracks]
        if not config.dry and len(to_add) > 0:
